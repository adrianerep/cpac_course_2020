...
float compute_flatness(FFT fft, float sum_of_spectrum){  //giving you input for each function
  float sumF = 0;
  for (int i = 0; i < fft.specSize(); i++){
    sumF += (fft.specSize() * (pow(fft.getBand(i), 1.0/fft.specSize())));
  }
float flatness = sumF/(0.001 + sum_of_spectrum);
   return flatness;               //RANDOMLY 0 BUT I THINK CORRECT BECAUSE "NOT FLAT"
}

float compute_centroid(FFT fft, float sum_of_spectrum, float[] freqs){
  float sum=0;
  for(int i = 0; i < fft.specSize(); i++){
    sum += freqs[i]*fft.getBand(i);      
  }  
  float centroid = sum/sum_of_spectrum;
  return centroid;
}
...
float compute_spread(FFT fft, float centroid, float sum_of_bands, float[] freqs){
  float sumS=0;
  for(int i = 0; i < fft.specSize(); i++){
    sumS += pow(freqs[i] - centroid, 2)*fft.getBand(i);      
  } 
  float spread = sumS/sum_of_bands;
  return spread;
}
...
float compute_skewness(FFT fft, float centroid, float spread, float[] freqs){
  float sumSk=0;
  for(int i = 0; i < fft.specSize(); i++){
    sumSk += pow(freqs[i] - centroid, 3)*fft.getBand(i);      
  } 
  float skewness = sumSk/ (0.001 + fft.specSize() *pow(spread,3)); //0.001 because if you divide by 0 it could become a problem
  return skewness;
}
...
float compute_entropy(FFT fft){
  float sumE=0;
  for(int i = 0; i < fft.specSize(); i++){
    sumE += fft.getBand(i)*(log(0.001 + fft.getBand(i)));      
  } 
  float entropy = sumE/log(0.001+ fft.specSize());
  return entropy;
}
...
class AgentFeature {  //AGENT FEATURE, EXTRACTS THE FEATURES
  int index_buffer=0;
  int index_spectrogram=0;
  int bufferSize;
  float sampleRate;
...
class AgentDrawer{    //AGENT DRAWER DRAWS FEATURES
  AgentFeature feat;
  int numFeatures;
  float heightFeatures;
  float maxWidth;
  ...
  void action(){
    ...
    for (int i=0; i<this.numFeatures; i++){
        ...
        if(this.wait_frames<=0){            
          ...
          */
            x=map(values[i], this.minFeatures[i], this.maxFeatures[i], 0, this.maxWidth);
         //take value and rescale to min and max then map from 0 to maximum length 
      }     
        /* this actually draws the value */
        rect(this.marginLeft, aboveSpace,  x, this.heightFeatures); //draws rect w length of feature
